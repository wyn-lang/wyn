...
```wyn
let x: int = pow(2, 8)   # 256
let y: int = pow(10, 3)  # 1000
```
...
let y: float = sqrt(2.0)   # 1.414...
```

#### `floor(n: float) -> int`
Round down to nearest integer.
```wyn
let x: int = floor(3.7)   # 3
let y: int = floor(-2.3)  # -3
```

#### `ceil(n: float) -> int`
Round up to nearest integer.
```wyn
let x: int = ceil(3.2)   # 4
let y: int = ceil(-2.7)  # -2
```

#### `round(n: float) -> int`
Round to nearest integer (0.5 rounds away from zero).
```wyn
let x: int = round(3.5)   # 4
let y: int = round(3.4)   # 3
let z: int = round(-2.5)  # -3
```
...
```wyn
let x: float = abs_float(-3.14)  # 3.14
```

#### `min_float(a: float, b: float) -> float`
Minimum of two floats.
```wyn
let x: float = min_float(3.14, 2.71)  # 2.71
```

#### `max_float(a: float, b: float) -> float`
Maximum of two floats.
```wyn
let x: float = max_float(3.14, 2.71)  # 3.14
```

#### `clamp(value: int, min: int, max: int) -> int`
Constrain value to range [min, max].
```wyn
let x: int = clamp(150, 0, 100)  # 100
let y: int = clamp(-10, 0, 100)  # 0
let z: int = clamp(50, 0, 100)   # 50
```

#### `clamp_float(value: float, min: float, max: float) -> float`
Constrain float to range.
```wyn
let x: float = clamp_float(1.5, 0.0, 1.0)  # 1.0
```
...
```wyn
let x: int = gcd(48, 18)  # 6
let y: int = gcd(100, 35) # 5
```

#### `lcm(a: int, b: int) -> int`
Least common multiple.
```wyn
let x: int = lcm(12, 18)  # 36
```

#### `factorial(n: int) -> int`
Calculate n! (n factorial).
```wyn
let x: int = factorial(5)  # 120
let y: int = factorial(0)  # 1
```
...
```wyn
let x: int = fibonacci(10)  # 55
let y: int = fibonacci(0)   # 0
let z: int = fibonacci(1)   # 1
```
...
```wyn
let x: int = digit_sum(123)  # 6 (1+2+3)
let y: int = digit_sum(999)  # 27
```

#### `digit_count(n: int) -> int`
Count number of digits.
```wyn
let x: int = digit_count(12345)  # 5
let y: int = digit_count(0)      # 1
```

#### `reverse_digits(n: int) -> int`
Reverse the digits of a number.
```wyn
let x: int = reverse_digits(123)  # 321
let y: int = reverse_digits(1000) # 1
```
...
```wyn
let x: float = lerp(0.0, 10.0, 0.5)  # 5.0
let y: float = lerp(0.0, 10.0, 0.25) # 2.5
let z: float = lerp(10.0, 20.0, 0.0) # 10.0
```
...
```wyn
let x: int = isqrt(16)  # 4
let y: int = isqrt(17)  # 4
```

#### `mod_pow(base: int, exp: int, m: int) -> int`
Modular exponentiation: (base^exp) % m.
```wyn
let x: int = mod_pow(2, 10, 1000)  # 24
```

#### `binomial(n: int, k: int) -> int`
Binomial coefficient "n choose k".
```wyn
let x: int = binomial(5, 2)  # 10
```

#### `totient(n: int) -> int`
Euler's totient function φ(n).
```wyn
let x: int = totient(9)  # 6
```

#### `mobius(n: int) -> int`
Möbius function μ(n).
```wyn
let x: int = mobius(6)  # 1
```
...
```wyn
let s: str = str_concat("hello", " world")  # "hello world"
```
...
```wyn
let s: str = trim_start("  hello  ")  # "hello  "
```

#### `trim_end(s: str) -> str`
Remove trailing whitespace only.
```wyn
let s: str = trim_end("  hello  ")  # "  hello"
```
...
```wyn
let pos: int = index_of("hello world", "world")  # 6
let not_found: int = index_of("hello", "xyz")    # -1
```

#### `last_index_of(s: str, substr: str) -> int`
Find last occurrence of substring.
```wyn
let pos: int = last_index_of("hello hello", "hello")  # 6
```

#### `count_substr(s: str, substr: str) -> int`
Count occurrences of substring.
```wyn
let n: int = count_substr("hello hello hello", "hello")  # 3
```
...
```wyn
let s: str = to_upper("hello")  # "HELLO"
```

#### `to_lower(s: str) -> str`
Convert string to lowercase.
```wyn
let s: str = to_lower("HELLO")  # "hello"
```

#### `capitalize(s: str) -> str`
Capitalize first character.
```wyn
let s: str = capitalize("hello")  # "Hello"
```
...
let s: str = join(parts, " ")  # "hello world !"
```
...
```wyn
let cmp: int = compare("apple", "banana")  # -1
```

#### `compare_ignore_case(a: str, b: str) -> int`
Case-insensitive comparison.
```wyn
let cmp: int = compare_ignore_case("Hello", "hello")  # 0
```
...
```wyn
let n: int = 7  # 0b111
let m: int = bit_clear(n, 1)  # 0b101 = 5
```
...
```wyn
let n: int = 5  # 0b101
let m: int = bit_toggle(n, 1)  # 0b111 = 7
```
...
```wyn
let n: int = 1  # 0b00000001
let r: int = reverse_bits(n)  # 0b10000000...
```
...
```wyn
let x: int = next_power_of_two(10)  # 16
let y: int = next_power_of_two(16)  # 16
```
...
```wyn
let n: int = 0b11110000
let bits: int = extract_bits(n, 4, 4)  # 0b1111 = 15
```
...
```wyn
let n: int = 0
let m: int = set_bits(n, 0, 4, 15)  # 0b1111 = 15
```
...
```wyn
let nums: [int] = [1, 2, 3]
let first: ?int = array_first(nums)
match first {
    some(n) => print(n)  # 1
    none => print("empty")
}
...
```wyn
let nums: [int] = [1, 2, 3]
let last: ?int = array_last(nums)  # some(3)
```
...
```wyn
let nums: [int] = [1, 2, 3]
let idx: int = array_index_of(nums, 2)  # 1
```

#### `array_count[T](arr: [T], item: T) -> int`
Count occurrences of item.
```wyn
let nums: [int] = [1, 2, 2, 3]
let count: int = array_count(nums, 2)  # 2
```
...
```wyn
let s: Stack[int] = stack_new[int]()
```

#### `stack_peek[T](s: Stack[T]) -> ?T`
Peek at top without removing.
```wyn
let s: Stack[int] = Stack[int] { items: [1, 2, 3] }
let top: ?int = stack_peek(s)  # some(3)
```
...
```wyn
let q: Queue[int] = queue_new[int]()
```

#### `queue_peek[T](q: Queue[T]) -> ?T`
Peek at front without removing.
```wyn
let q: Queue[int] = Queue[int] { items: [1, 2, 3] }
let front: ?int = queue_peek(q)  # some(1)
```
...
```wyn
let path: str = join_path("/home/user", "file.txt")
# "/home/user/file.txt"
```
...
```wyn
let parts: [str] = ["/home", "user", "docs", "file.txt"]
let path: str = join_paths(parts)
...
```wyn
let dir: str = dirname("/home/user/file.txt")  # "/home/user"
let d2: str = dirname("file.txt")  # "."
```

#### `basename(path: str) -> str`
Get filename portion (last component).
```wyn
let name: str = basename("/home/user/file.txt")  # "file.txt"
```

#### `extension(path: str) -> str`
Get file extension including dot.
```wyn
let ext: str = extension("file.txt")  # ".txt"
let e2: str = extension("archive.tar.gz")  # ".gz"
```

#### `stem(path: str) -> str`
Get filename without extension.
```wyn
let stem: str = stem("file.txt")  # "file"
```

#### `parent(path: str) -> str`
Get parent directory (alias for dirname).
```wyn
let p: str = parent("/home/user/file.txt")  # "/home/user"
```
...
```wyn
let depth: int = path_depth("/home/user/docs")  # 3
```
...
```wyn
let clean: str = normalize_path("/home//user///file.txt")
# "/home/user/file.txt"
```
...
```wyn
let path: str = remove_extension("file.txt")  # "file"
```

#### `with_extension(path: str, ext: str) -> str`
Replace extension with new one.
```wyn
let path: str = with_extension("file.txt", ".md")
# "file.md"
```
...
```wyn
let path: str = ensure_trailing_slash("/home/user")
# "/home/user/"
```
...
```wyn
let path: str = remove_trailing_slash("/home/user/")
# "/home/user"
```
...
fn main() {
    let x: int = abs(-42)  # Direct call, no prefix
}
```
...
```wyn
let x: int = abs(-42)        # ✅ OK
let y: float = sqrt(16.0)    # ✅ OK
let z: int = sqrt(16)        # ❌ Type error
```
