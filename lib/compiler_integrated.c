// Integrated Wyn Compiler - Generated from C components
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

// === From lib/lexer.wyn.c ===
...
#define TokenType_EndOfFile 0
#define TokenType_Ident 1
#define TokenType_Int 2
#define TokenType_String 3
#define TokenType_Fn 4
#define TokenType_Var 5
#define TokenType_If 6
#define TokenType_Else 7
#define TokenType_While 8
#define TokenType_Return 9
#define TokenType_Plus 10
#define TokenType_Minus 11
#define TokenType_Star 12
#define TokenType_Slash 13
#define TokenType_Equal 14
#define TokenType_EqualEqual 15
#define TokenType_Less 16
#define TokenType_Greater 17
#define TokenType_Arrow 18
#define TokenType_Dot 19
#define TokenType_Colon 20
#define TokenType_LParen 21
#define TokenType_RParen 22
#define TokenType_LBrace 23
#define TokenType_RBrace 24
#define TokenType_Comma 25
#define TokenType_Semicolon 26
#define TokenType_Unknown 27

const char* TokenType_toString(TokenType val) {
    switch(val) {
...
TokenType check_keyword(const char* input, int start, int len);
char* substr(const char* s, int start, int len);
Token make_token(TokenType tok_type, const char* value);
WynArray lex(const char* input);
int wyn_main();
...
TokenType check_keyword(const char* input, int start, int len) {
    if ((len == 2)) {
    if ((strcmp(wyn_string_charat(input, start), "f") == 0)) {
    if ((strcmp(wyn_string_charat(input, (start + 1)), "n") == 0)) {
    return TokenType_Fn;
    }
    }
    }
    if ((len == 2)) {
    if ((strcmp(wyn_string_charat(input, start), "i") == 0)) {
    if ((strcmp(wyn_string_charat(input, (start + 1)), "f") == 0)) {
    return TokenType_If;
    }
    }
    }
    if ((len == 3)) {
    if ((strcmp(wyn_string_charat(input, start), "v") == 0)) {
    if ((strcmp(wyn_string_charat(input, (start + 1)), "a") == 0)) {
    if ((strcmp(wyn_string_charat(input, (start + 2)), "r") == 0)) {
...
char* substr(const char* s, int start, int len) {
    ...
    int i = 0;
    while ((i < len)) {
    result = wyn_string_concat_safe(result, wyn_string_charat(s, (start + i)));
    i = (i + 1);
    }
    return result;
}
...
Token make_token(TokenType tok_type, const char* value) {
    Token tok = *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = tok_type, .value = value})->data;
    return tok;
}

WynArray lex(const char* input) {
    int pos = 0;
    WynArray tokens = ({ WynArray __arr_0 = array_new(); __arr_0; });
    while ((pos < string_len(input))) {
    const char* ch = wyn_string_charat(input, pos);
    if (is_whitespace(ch)) {
    pos = (pos + 1);
    } else if (is_digit(ch)) {
    int start = pos;
    while ((pos < string_len(input))) {
    if (is_digit(wyn_string_charat(input, pos))) {
    pos = (pos + 1);
    } else {
    break;
    }
    }
    array_push_struct(&(tokens), make_token(TokenType_Int, substr(input, start, (pos - start))), Token);
    } else if (is_alpha(ch)) {
    int start = pos;
    while ((pos < string_len(input))) {
    const char* c = wyn_string_charat(input, pos);
    if (is_alpha(c)) {
    pos = (pos + 1);
    } else {
    break;
    ...
    array_push_struct(&(tokens), make_token(check_keyword(input, start, (pos - start)), substr(input, start, (pos - start))), Token);
    } else if ((strcmp(ch, "+") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_Plus, "+"), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, "-") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_Minus, "-"), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, "*") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_Star, "*"), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, "/") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_Slash, "/"), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, "=") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_Equal, "="), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, "<") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_Less, "<"), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, ">") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_Greater, ">"), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, ".") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_Dot, "."), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, ":") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_Colon, ":"), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, "(") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_LParen, "("), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, ")") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_RParen, ")"), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, "{") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_LBrace, "{"), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, "}") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_RBrace, "}"), Token);
    pos = (pos + 1);
    } else if ((strcmp(ch, ";") == 0)) {
    array_push_struct(&(tokens), make_token(TokenType_Semicolon, ";"), Token);
    pos = (pos + 1);
    } else {
    array_push_struct(&(tokens), make_token(TokenType_Unknown, ch), Token);
    pos = (pos + 1);
    }
    }
    array_push_struct(&(tokens), make_token(TokenType_EndOfFile, ""), Token);
    return tokens;
}

int wyn_main() {
    const char* input = "var x = 42";
    __auto_type tokens = lex(input);
    print("Tokenization test:");
    print("Input: var x = 42");
    print("Token count: ");
    print(array_len(tokens));
    if ((array_len(tokens) == 5)) {
...
#define TokenType_Int 2
#define TokenType_String 3
#define TokenType_Var 4
#define TokenType_Return 5
#define TokenType_Colon 6
#define TokenType_Equal 7
#define TokenType_Plus 8
#define TokenType_Minus 9
#define TokenType_Star 10
#define TokenType_Slash 11
#define TokenType_EqualEqual 12
#define TokenType_BangEqual 13
#define TokenType_Less 14
#define TokenType_Greater 15
#define TokenType_LessEqual 16
#define TokenType_GreaterEqual 17
#define TokenType_LeftParen 18
#define TokenType_RightParen 19
#define TokenType_LeftBrace 20
#define TokenType_RightBrace 21
#define TokenType_Comma 22
#define TokenType_Fn 23
#define TokenType_If 24
#define TokenType_Else 25
#define TokenType_While 26
#define TokenType_Arrow 27
#define TokenType_Struct 28
#define TokenType_Enum 29
#define TokenType_Match 30
#define TokenType_DoubleColon 31
...
#define NodeType_NodeInt 0
#define NodeType_NodeString 1
#define NodeType_NodeVariable 2
#define NodeType_NodeVarDecl 3
#define NodeType_NodeReturn 4
#define NodeType_NodeExprStmt 5
#define NodeType_NodeBinary 6
#define NodeType_NodeFnDecl 7
#define NodeType_NodeBlock 8
#define NodeType_NodeIf 9
#define NodeType_NodeWhile 10
#define NodeType_NodeCall 11
#define NodeType_NodeStructDecl 12
#define NodeType_NodeEnumDecl 13
#define NodeType_NodeMatch 14
#define NodeType_NodeStructInit 15

const char* NodeType_toString(NodeType val) {
    switch(val) {
        case NodeInt: return "NodeInt";
        case NodeString: return "NodeString";
        case NodeVariable: return "NodeVariable";
        case NodeVarDecl: return "NodeVarDecl";
        case NodeReturn: return "NodeReturn";
        case NodeExprStmt: return "NodeExprStmt";
        case NodeBinary: return "NodeBinary";
        case NodeFnDecl: return "NodeFnDecl";
        case NodeBlock: return "NodeBlock";
        case NodeIf: return "NodeIf";
        case NodeWhile: return "NodeWhile";
        case NodeCall: return "NodeCall";
        case NodeStructDecl: return "NodeStructDecl";
        case NodeEnumDecl: return "NodeEnumDecl";
        case NodeMatch: return "NodeMatch";
        case NodeStructInit: return "NodeStructInit";
    }
    return "Unknown";
}
...
void ASTNode_cleanup(ASTNode* obj) {
}
...
int parse_match();
int parse_stmt();
WynArray parse(WynArray tokens);
int wyn_main();

Token peek() {
    if ((g_current < array_len(g_tokens))) {
    return array_get_struct(g_tokens, g_current, Token);
    }
    return *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_EndOfFile, .value = ""})->data;
}

int check(TokenType tok_type) {
    __auto_type tok = peek();
...
int add_node(ASTNode node) {
    int idx = array_len(g_nodes);
    array_push_struct(&(g_nodes), node, ASTNode);
    return idx;
}
...
int parse_primary() {
    ...
    advance();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeInt, .value = tok.value, .left = -1, .right = -1, .extra = -1})->data);
    }
    if ((tok.tok_type == TokenType_String)) {
    advance();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeString, .value = tok.value, .left = -1, .right = -1, .extra = -1})->data);
    }
    if ((tok.tok_type == TokenType_Ident)) {
    advance();
    __auto_type idx = add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeVariable, .value = tok.value, .left = -1, .right = -1, .extra = -1})->data);
    if (check(TokenType_LeftParen)) {
    advance();
    WynArray args = ({ WynArray __arr_2 = array_new(); __arr_2; });
    if ((check(TokenType_RightParen) == 0)) {
    __auto_type arg_idx = parse_expr();
    array_push(&(args), arg_idx);
    ...
    advance();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeCall, .value = "", .left = idx, .right = array_len(args), .extra = -1})->data);
    }
    if (check(TokenType_LeftBrace)) {
    advance();
    while (((check(TokenType_RightBrace) == 0) && (check(TokenType_EndOfFile) == 0))) {
    __auto_type field_tok = peek();
    advance();
    if (check(TokenType_Colon)) {
    advance();
    __auto_type val_idx = parse_expr();
    }
    ...
    advance();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeStructInit, .value = tok.value, .left = -1, .right = -1, .extra = -1})->data);
    }
    return idx;
    }
    if ((tok.tok_type == TokenType_LeftParen)) {
    advance();
    __auto_type expr_idx = parse_expr();
    advance();
    return expr_idx;
    }
    return -1;
...
int parse_comparison() {
    __auto_type left = parse_primary();
    ...
    __auto_type op_tok = peek();
    advance();
    __auto_type right = parse_primary();
    left = add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeBinary, .value = op_tok.value, .left = left, .right = right, .extra = -1})->data);
    }
    return left;
}
...
int parse_term() {
    __auto_type left = parse_factor();
    while ((check(TokenType_Plus) || check(TokenType_Minus))) {
    __auto_type op_tok = peek();
    advance();
    __auto_type right = parse_factor();
    left = add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeBinary, .value = op_tok.value, .left = left, .right = right, .extra = -1})->data);
    }
    return left;
}
...
int parse_var_decl() {
    ...
    advance();
    __auto_type expr_idx = parse_expr();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeVarDecl, .value = name_tok.value, .left = expr_idx, .right = -1, .extra = -1})->data);
}

int parse_block() {
    advance();
    WynArray stmts = ({ WynArray __arr_3 = array_new(); __arr_3; });
    while (((check(TokenType_RightBrace) == 0) && (check(TokenType_EndOfFile) == 0))) {
    __auto_type stmt_idx = parse_stmt();
    if ((stmt_idx >= 0)) {
    array_push(&(stmts), stmt_idx);
    }
    }
    advance();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeBlock, .value = "", .left = -1, .right = array_len(stmts), .extra = -1})->data);
}

int parse_if() {
    advance();
    __auto_type cond_idx = parse_expr();
    __auto_type then_idx = parse_block();
    int else_idx = -1;
    ...
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeIf, .value = "", .left = cond_idx, .right = then_idx, .extra = else_idx})->data);
}

int parse_while() {
    advance();
    __auto_type cond_idx = parse_expr();
    __auto_type body_idx = parse_block();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeWhile, .value = "", .left = cond_idx, .right = body_idx, .extra = -1})->data);
}
...
int parse_struct_decl() {
    ...
    advance();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeStructDecl, .value = name_tok.value, .left = -1, .right = -1, .extra = -1})->data);
}
...
int parse_enum_decl() {
    ...
    advance();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeEnumDecl, .value = name_tok.value, .left = -1, .right = -1, .extra = -1})->data);
}
...
int parse_fn_decl() {
    ...
    __auto_type body_idx = parse_block();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeFnDecl, .value = name_tok.value, .left = body_idx, .right = -1, .extra = -1})->data);
}
...
int parse_match() {
    ...
    advance();
    while (((check(TokenType_RightBrace) == 0) && (check(TokenType_EndOfFile) == 0))) {
    __auto_type pattern_idx = parse_expr();
    if (check(TokenType_Arrow)) {
    advance();
    }
    __auto_type body_idx = parse_stmt();
    if (check(TokenType_Comma)) {
    advance();
    }
    }
    advance();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeMatch, .value = "", .left = expr_idx, .right = -1, .extra = -1})->data);
}
...
int parse_stmt() {
    ...
    advance();
    __auto_type expr_idx = parse_expr();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeReturn, .value = "", .left = expr_idx, .right = -1, .extra = -1})->data);
    }
    ...
    __auto_type expr_idx = parse_expr();
    return add_node(*(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeExprStmt, .value = "", .left = expr_idx, .right = -1, .extra = -1})->data);
}

WynArray parse(WynArray tokens) {
    g_tokens = tokens;
    g_current = 0;
    g_nodes = ({ WynArray __arr_4 = array_new(); __arr_4; });
    WynArray decls = ({ WynArray __arr_5 = array_new(); __arr_5; });
    while ((check(TokenType_EndOfFile) == 0)) {
    __auto_type tok = peek();
    if ((tok.tok_type == TokenType_Fn)) {
    __auto_type fn_idx = parse_fn_decl();
    if ((fn_idx >= 0)) {
    array_push(&(decls), fn_idx);
    }
    } else if ((tok.tok_type == TokenType_Struct)) {
    __auto_type struct_idx = parse_struct_decl();
    if ((struct_idx >= 0)) {
    array_push(&(decls), struct_idx);
    }
    } else if ((tok.tok_type == TokenType_Enum)) {
    __auto_type enum_idx = parse_enum_decl();
    if ((enum_idx >= 0)) {
    array_push(&(decls), enum_idx);
    }
    } else {
    __auto_type stmt_idx = parse_stmt();
    if ((stmt_idx >= 0)) {
    array_push(&(decls), stmt_idx);
    }
    }
    }
    return decls;
...
int wyn_main() {
    ...
    WynArray tokens = ({ WynArray __arr_6 = array_new(); __arr_6; });
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_Struct, .value = "struct"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_Ident, .value = "Point"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_LeftBrace, .value = "{"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_Ident, .value = "x"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_Colon, .value = ":"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_Ident, .value = "int"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_Comma, .value = ","})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_Ident, .value = "y"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_Colon, .value = ":"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_Ident, .value = "int"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_RightBrace, .value = "}"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = TokenType_EndOfFile, .value = ""})->data, Token);
    __auto_type decls = parse(tokens);
    if ((array_len(decls) > 0)) {
...
#define SymbolKind_Var 0
#define SymbolKind_Fn 1
#define SymbolKind_Param 2

const char* SymbolKind_toString(SymbolKind val) {
    switch(val) {
        case Var: return "Var";
        case Fn: return "Fn";
...
#define ExprKind_Int 0
#define ExprKind_String 1
#define ExprKind_Ident 2
#define ExprKind_Binary 3
#define ExprKind_Call 4
#define ExprKind_Unary 5

const char* ExprKind_toString(ExprKind val) {
    switch(val) {
        case Int: return "Int";
        case String: return "String";
        case Ident: return "Ident";
...
#define StmtKind_VarDecl 0
#define StmtKind_FnDecl 1
#define StmtKind_Return 2
#define StmtKind_If 3
#define StmtKind_While 4
#define StmtKind_Expr 5

const char* StmtKind_toString(StmtKind val) {
    switch(val) {
        case VarDecl: return "VarDecl";
        case FnDecl: return "FnDecl";
        case Return: return "Return";
        case If: return "If";
        case While: return "While";
...
int g_error_count = 0;
int g_in_function = 0;
const char* g_current_fn_ret_type = "";

// Lambda functions (defined before use)
int checker_error(const char* msg);
int enter_scope();
int exit_scope();
int add_symbol(const char* name, SymbolKind kind, const char* typ);
int find_symbol(const char* name);
char* get_symbol_type(int idx);
int types_equal(const char* t1, const char* t2);
char* check_binary_expr(const char* left_type, const char* op, const char* right_type);
char* check_expr(ExprKind kind, const char* value);
int check_stmt(StmtKind kind, const char* name, const char* typ);
int check_return_type(const char* ret_type);
int end_function();
int end_block();
int check_var_decl(const char* name, const char* typ);
int check_fn_decl(const char* name, const char* ret_type);
int check_if_stmt();
int check_while_stmt();
int check_return_stmt(const char* ret_type);
int check_condition(const char* cond_type);
int check_program();
int wyn_main();
...
int wyn_main() {
    ...
    int param1 = add_symbol("a", SymbolKind_Param, "int");
    int param2 = add_symbol("b", SymbolKind_Param, "int");
    const char* left_type = check_expr(ExprKind_Ident, "a");
    const char* right_type = check_expr(ExprKind_Ident, "b");
    const char* result_type = check_binary_expr(left_type, "+", right_type);
    check_return_stmt(result_type);
    end_function();
    check_fn_decl("test", "void");
    check_var_decl("z", "int");
    const char* z_type = check_expr(ExprKind_Ident, "z");
    const char* cond_type = check_binary_expr(z_type, ">", "int");
    check_condition(cond_type);
    check_if_stmt();
    const char* expr_type = check_expr(ExprKind_Int, "42");
    end_block();
    check_while_stmt();
    const char* loop_cond = check_binary_expr("int", "<", "int");
    check_condition(loop_cond);
    end_block();
    end_function();
    int exit_code = check_program();
}


// === From lib/codegen.wyn.c ===
#define _POSIX_C_SOURCE 200809L
...
int codegen_int(ASTNode node);
int codegen_string(ASTNode node);
int codegen_variable(ASTNode node);
int codegen_binary(ASTNode node);
int codegen_call(ASTNode node);
int codegen_struct_init(ASTNode node);
int codegen_expr(int idx);
int codegen_var_decl(ASTNode node);
int codegen_return(ASTNode node);
int codegen_expr_stmt(ASTNode node);
int codegen_if(ASTNode node);
int codegen_while(ASTNode node);
int codegen_block(ASTNode node);
int codegen_fn_decl(ASTNode node);
int codegen_struct_decl(ASTNode node);
int codegen_enum_decl(ASTNode node);
int codegen_match(ASTNode node);
char* codegen_program(int root_idx);
int codegen_stmt(int idx);
int wyn_main();

int emit(const char* s) {
    g_output = wyn_string_concat_safe(g_output, s);
    return 0;
}
...
int codegen_call(ASTNode node) {
    ASTNode fn_node = array_get_struct(g_nodes, node.left, ASTNode);
    emit(fn_node.value);
    emit("(");
    int arg_idx = node.right;
    int first = 1;
    while ((arg_idx >= 0)) {
    if ((first == 0)) {
    emit(", ");
    ...
    ASTNode arg_node = array_get_struct(g_nodes, arg_idx, ASTNode);
...
int codegen_struct_init(ASTNode node) {
    ...
    ASTNode field_node = array_get_struct(g_nodes, field_idx, ASTNode);
...
int codegen_expr(int idx) {
    ...
    ASTNode node = array_get_struct(g_nodes, idx, ASTNode);
...
int codegen_block(ASTNode node) {
    ...
    while ((i >= 0)) {
    codegen_stmt(i);
    ASTNode stmt = array_get_struct(g_nodes, i, ASTNode);
    i = stmt.extra;
    }
    return 0;
}
...
int codegen_fn_decl(ASTNode node) {
    ...
    first = 0;
    ASTNode param = array_get_struct(g_nodes, param_idx, ASTNode);
    emit("int ");
    emit(param.value);
...
int codegen_struct_decl(ASTNode node) {
    ...
    int field_idx = node.left;
    while ((field_idx >= 0)) {
    ASTNode field = array_get_struct(g_nodes, field_idx, ASTNode);
    emit("    int ");
    emit(field.value);
    emit(";\n");
...
int codegen_enum_decl(ASTNode node) {
    ...
    first = 0;
    ASTNode variant = array_get_struct(g_nodes, variant_idx, ASTNode);
    emit("    ");
    emit(node.value);
    emit("_");
    emit(variant.value);
...
int wyn_main() {
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeInt, .value = "42", .left = -1, .right = -1, .extra = -1})->data, ASTNode);
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeReturn, .value = "", .left = 0, .right = -1, .extra = -1})->data, ASTNode);
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeBlock, .value = "", .left = 1, .right = -1, .extra = -1})->data, ASTNode);
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeFnDecl, .value = "test", .left = -1, .right = 2, .extra = -1})->data, ASTNode);
    __auto_type result = codegen_program(3);
    print("✓ Function codegen works!");
    g_output = "";
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeVariable, .value = "x", .left = -1, .right = -1, .extra = 5})->data, ASTNode);
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeVariable, .value = "y", .left = -1, .right = -1, .extra = -1})->data, ASTNode);
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeStructDecl, .value = "Point", .left = 4, .right = -1, .extra = -1})->data, ASTNode);
    codegen_stmt(6);
    print("✓ Struct codegen works!");
    g_output = "";
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeVariable, .value = "Red", .left = -1, .right = -1, .extra = 8})->data, ASTNode);
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeVariable, .value = "Blue", .left = -1, .right = -1, .extra = -1})->data, ASTNode);
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = NodeType_NodeEnumDecl, .value = "Color", .left = 7, .right = -1, .extra = -1})->data, ASTNode);
    codegen_stmt(9);
