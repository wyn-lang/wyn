...
const char* Color_toString(Color val) {
    switch(val) {
        case Red: return "Red";
...
void Box_cleanup(Box* obj) {
}

/* Generating 0 methods */

// Lambda functions (defined before use)
int check_color(Box b, Color expected);
int wyn_main();
...
int wyn_main() {
    Box b = *(Box*)wyn_arc_new(sizeof(Box), &(Box){.color = Color_Red})->data;
    __auto_type result = check_color(b, Color_Red);
    print(result);
    return 0;
}
