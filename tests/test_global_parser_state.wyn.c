...
int add_token(int tok_type, const char* value);
Token peek();
int advance();
int wyn_main();

int add_token(int tok_type, const char* value) {
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = tok_type, .value = value})->data, Token);
}

Token peek() {
    if ((current < array_len(tokens))) {
    return array_get_struct(tokens, current, Token);
    }
    return *(Token*)wyn_arc_new(sizeof(Token), &(Token){.tok_type = 0, .value = ""})->data;
}
