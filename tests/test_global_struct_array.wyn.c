...
char* test_global_access(int idx);
int wyn_main();

char* test_global_access(int idx) {
    int node = array_get_int(g_nodes, idx);
    return node.value;
}

int wyn_main() {
    array_push_struct(&(g_nodes), *(ASTNode*)wyn_arc_new(sizeof(ASTNode), &(ASTNode){.node_type = 1, .value = "test"})->data, ASTNode);
    __auto_type result = test_global_access(0);
    print(result);
    return 0;
}
