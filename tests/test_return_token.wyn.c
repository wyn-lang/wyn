...
void Parser_cleanup(Parser* obj) {
}
...
Token parser_peek(Parser p) {
    int len = array_len(p.tokens);
    return *(Token*)wyn_arc_new(sizeof(Token), &(Token){.value = len})->data;
}

int wyn_main() {
    WynArray tokens = ({ WynArray __arr_0 = array_new(); __arr_0; });
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.value = 1})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.value = 2})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.value = 3})->data, Token);
    Parser p = *(Parser*)wyn_arc_new(sizeof(Parser), &(Parser){.tokens = tokens, .current = 0})->data;
    __auto_type tok = parser_peek(p);
    return tok.value;
}
