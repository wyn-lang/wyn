...
void set_insert(WynHashSet* set, int value) {
    wyn_hashset_insert(set, &value);
}
bool set_contains(WynHashSet* set, int value) {
    return wyn_hashset_contains(set, &value);
}
void set_remove(WynHashSet* set, int value) {
    wyn_hashset_remove(set, &value);
}
int set_len(WynHashSet* set) {
    return (int)wyn_hashset_len(set);
}
bool set_is_empty(WynHashSet* set) {
    return wyn_hashset_is_empty(set);
}
...
int std_io_println(const char* msg);
int std_io_print_str(const char* msg);
int std_io_print_int(int n);
const char* std_io_read_file(const char* path);
int std_io_write_file(const char* path, const char* content);
int std_io_file_exists(const char* path);
int std_io_delete_file(const char* path);
int std_io_list_dir(const char* path);
int std_io_create_dir(const char* path);
const char* std_io_get_cwd();
int std_io_println(const char* msg) {
    print(msg);
    print("\n");
    return 0;
...
int wyn_main() {
    std_io_println("Hello from std/io");
    std_io_print("Count: ");
    std_io_println("42");
    __auto_type content = std_io_read_file("test.txt");
    ;
    std_io_println(wyn_string_concat_safe("File content: ", int_to_string(content)));
    __auto_type result = std_io_write_file("output.txt", "Test content");
    ;
    if ((result == 0)) {
    std_io_println("Write successful");
    }
    return 0;
}
