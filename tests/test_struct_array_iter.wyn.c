...
int wyn_main() {
    ...
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.value = "hello"})->data, Token);
    array_push_struct(&(tokens), *(Token*)wyn_arc_new(sizeof(Token), &(Token){.value = "world"})->data, Token);
    int i = 0;
    int len = array_len(tokens);
    while ((i < len)) {
    Token tok = array_get_struct(tokens, i, Token);
    process_token(tok);
    i = (i + 1);
    }
}
