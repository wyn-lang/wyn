...
int wyn_main() {
    ...
    ;
    __auto_type s = add(p.x, p.y);
    ;
    __auto_type v = id_int(s);
    ;
    WynArray a = ({ WynArray __arr_0 = array_new(); array_push_int(&__arr_0, 10); array_push_int(&__arr_0, 20); array_push_int(&__arr_0, 30); __arr_0; });
    ;
    int m = ({ int _match_result_0; switch (v) {
        case 15: _match_result_0 = array_get_int(a, 2); break;
        default: _match_result_0 = 0; break;
    } _match_result_0; });
    ;
    WynOptional* o = ({ some_int(m); /* ARC retain for o */ });
    ;
    switch (m) {
        case 30: return 30;
 break;
