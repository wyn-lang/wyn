...
int maybe_value() {
    ...
    ;
    WynOptional* y = ({ wyn_none(); /* ARC retain for y */ });
    ;
    return 1;
    if(x) free(x);
    if(y) free(y);
...
int wyn_main() {
    ...
    ;
    int arr_val = array_get_int(arr, 1);
    ;
    int match_result = ({ int _match_result_0; switch (sum) {
        case 15: _match_result_0 = arr_val; break;
        default: _match_result_0 = 0; break;
    } _match_result_0; });
    ;
    switch (p.x) {
        case 3: return match_result;
 break;
        default: return 0;
 break;
    }
