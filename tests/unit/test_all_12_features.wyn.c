...
void* __spawn_wrapper_worker(void* arg) {
    worker();
    return NULL;
}
...
int wyn_main() {
    ...
    ;
    int matched = ({ int _match_result_0; switch (p.x) {
        case 3: _match_result_0 = 77; break;
    ...
    ;
    wyn_spawn(__spawn_wrapper_worker, NULL);
    return matched;
    if(ok_val) free(ok_val);
    if(some_val) free(some_val);
}
